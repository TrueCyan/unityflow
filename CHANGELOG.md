...
- Quick Test 버튼(Ping/Screenshot/Hierarchy) 클릭 시 데드락 발생하던 문제 수정
  - Main Thread에서 동기 HTTP 요청 → `ExecuteOnMainThread` 대기 순환으로 인한 데드락
  - HTTP 요청을 ThreadPool에서 비동기 실행하도록 변경
...
- Unity 패키지에 `.meta` 파일 누락으로 UPM 임포트 불가하던 문제 수정
